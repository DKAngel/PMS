...
<body>
	...
	<div class="panel ">
        <div class="panel-heading">
        	<h4>建议人：${ suggestOwner.ownersName }</h4>
        	<h4>电话：${ suggestOwner.ownersPhone }</h4>
        	<h4>房间：${ suggestOwner.roomId }</h4>
           	<h4>建议时间：<fmt:formatDate value = "${suggest.suggestTime}" pattern="yyyy-MM-dd HH:mm:ss"/></h4>
           	<h4>建议内容：</h4>
           	<h5>${ suggest.suggestContent }</h5>
        </div>
		<hr>
        <div class="panel-heading">
            <h4 style="text-align:left">处理人：${ adminName }</h4>
           	...
           	<form action="${pageContext.request.contextPath}/adminHandle/updateSuggest/${suggest.suggestId}" method="post">
           		
           		<textarea class="form-control" name="suggestResult" rows="4" >${ suggest.suggestHandleResult }</textarea>
